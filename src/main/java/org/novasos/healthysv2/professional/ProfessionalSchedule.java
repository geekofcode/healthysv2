...
class ProfessionalSchedule {
 ...
 @Column(name="day_of_week",nullable=false) private int dayOfWeek; @Column(name="start_time",nullable=false) private LocalTime startTime; @Column(name="end_time",nullable=false) private LocalTime endTime; @Column(name="slot_duration_minutes",nullable=false) private int slotDurationMinutes;
 ...
 void update(int d,LocalTime s,LocalTime e,int duration){if(d<1||d>7)throw new IllegalArgumentException("Invalid day");if(s==null||e==null||!e.isAfter(s))throw new IllegalArgumentException("Invalid schedule times");if(duration<=0)throw new IllegalArgumentException("Invalid slot duration");dayOfWeek=d;startTime=s;endTime=e;slotDurationMinutes=duration;}
